...
void setup() {
  ...
  ledcAttachChannel(pwmA, 30000, 8, 0);  //pwm频道, 频率, 精度
  ledcAttachChannel(pwmB, 30000, 8, 1);  //pwm频道, 频率, 精度
  ledcAttachChannel(pwmC, 30000, 8, 2);  //pwm频道, 频率, 精度
...
在测试过程中，发现电机会抖动。修改和调试了很多软硬件问题才发现，原来问题出在这几行：
...
由于在每次循环里，都输出了调试信息到串口，导致实时性有问题。

因此做了Serial输出时间的调试：

```
void checkPrintTime(int num) {
  unsigned long now_us = micros();
  for (int i = 0; i < num; i++) {
    Serial.println(num);
  }
  unsigned long end_us = micros();
    Serial.println(end_us-now_us);
}
...
输出1次、5次、10次的时间分别为41、169、339 微秒。
而测试每一次循环的时间约为23微秒，确实println太消耗时间了，在实时性要求高的场景下是不能使用的。
